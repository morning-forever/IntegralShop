					...
					<tr>
						...
						<td colspan="3">
							<select name="from_type" id="from_type">
								<option value="cart">快旅出行banner</option>
							</select>
						</td>
					</tr>
					<tr>
...
<div class="modal fade" id="advertModel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
...
function initSelectList(){
	$('#select_table').DataTable({
		"columns": [
            ...
            "columnDefs": [{
            	...
            	render: function (data, type, row, d) {
            		...
            		{
            			func: [
            			{"name": "选择", "fn": "doSelect(\'" + data.id + "\',\'" + data.title + "\')", "type": "purple", "icon":"plus"}
            			]
            		};
		...
		"bDestroy": true,
		"ajax": {
			url:"admin/cart/banner/getSelectList",
			type: "POST"
		}
	});
}
...
function doSubmit(){
	...
	if(checkForm(ids)){
		...
		$.post("admin/cart/banner/submit",data,function (req) {
				if(req.status=="success"){
					bootbox.alert("提交成功",function(){
						window.location.href="admin/cart/banner/index";	
					})
