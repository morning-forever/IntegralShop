...
function onLine(type){
	...
	var url="";
	// if(type==1){
		url="admin/home/banner/changeEnable";
	// }else{
	// 	url="";
	// }
	$("input[flag=row]").each(function(){
...
function doAdd(type){
	window.location.href="admin/home/banner/add?type="+type;
}

function doEdit(id,type){
	window.location.href="admin/home/banner/edit_"+id+"_"+type;
}
...
function initMainList(id,RBType){
	...
	$('#'+id).DataTable( {
		...
		"bDestroy": true,
		"ajax": {
			url:"admin/home/banner/recommendList",
			data:{type:RBType},
			type: "POST"
		}
	} );
