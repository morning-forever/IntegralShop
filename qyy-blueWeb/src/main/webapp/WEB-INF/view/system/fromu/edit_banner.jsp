					...
					<tr>
						...
						<td colspan="3">
							<select id="from_table" name="from_table">
								<option value="1">精彩游记</option>
								<option value="2">达人攻略</option>
								<option value="3">玩伴招募</option>
							</select>
						</td>
					</tr>
					<tr>
		...
		"columns": [
            ...
            "columnDefs": [{
            	...
            	render: function (data, type, row, d) {
            		...
            		{
            			func: [
            			{"name": "选择", "fn": "doSelect(\'" + row.id + "\',\'" + row.title + "\')", "type": "purple", "icon":"plus"}
            			]
            		};
			...
			$('#select_table').DataTable().column(5).visible(false);//隐藏ID行
			},
		"oLanguage":{
...
function doSubmit(){
	...
	if(checkForm(ids)){
		...
		$.post("admin/fromu/submit",data,function(req){
			console.info(req.status);
			if(req.status=="success"){
				bootbox.alert("提交成功",function(){
					window.location.href="admin/fromu/index";	
				})
			}else if(req.status=="exists"){
				bootbox.alert("已有相同位置的推荐，请确认");
			}else{
				bootbox.alert("提交失败，请重试");
			}
		});
		/* $("#form").ajaxSubmit({
			url:"admin/fromu/submit",
			type:'post',
...
function initData(){
	...
	$("#from_table").val("${map.banner_link-5}");
