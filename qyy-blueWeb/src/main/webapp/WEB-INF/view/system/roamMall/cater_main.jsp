	...
	<div class="container-fluid" id="main-container">
		<div id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active">餐饮管理</li>
			</ul>
			<!--.breadcrumb-->
		</div>
		<!--#breadcrumbs-->
		<div class="row-fluid">
			<div class="table-header">餐饮管理</div>
			<ul class="nav nav-tabs" id="otherInfoTab">
				<li class="active"><a href="#tab_1">餐饮分类</a></li>
				<li><a href="#tab_2">餐饮价格筛选</a></li>
			</ul>
				...
				<div class="tab-pane active" id="tab_1">
					<table style="margin-left: 5px;" id="btnmenu">
						<tbody>
							<tr>
								<td>分类名称：</td>
								<td><select id="sCaterName" style="width: 150px">
										<option value="">--请选择--</option>
										<c:forEach items="${caterList }" var="cater">
								...
								<td><a title="添加" href="javascript:openCaterModel('new');"
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div>
							<form id="myform1">
								<input type="hidden" id="caterId" name="caterId" />
								<table class="mytable-detail">
									<tr>
										<th>分类名称：</th>
										<td><input type="text" class="form-control"
											maxlength="20" id="caterName" name="caterName"
					...
					<div class="modal-body">
						<div>
							<form id="myform2">
								...
								<input type="hidden" id="searchType" name="searchType" value="cater"/>
								<table class="mytable-detail">
									<tr>
										<th>餐饮价格从：</th>
										<td><input type="text" class="form-control"
											maxlength="20" id="priceStart" name="priceStart"
											placeholder="餐饮价格从"></td>
										<th>至：</th>
										<td><input type="text" class="form-control"
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#searchBtn1").click(function() {
			var caterId = $("#sCaterName").val();

			var json = '{"caterId":"' + caterId + '"}';
		...
		$("#searchBtn2").click(function() {
			...
			var json = '{"priceId":"' + priceId + '","searchType":"cater"}';
			...
			$.ajax({
				...
				success : function(req) {
					if (req.status == "success") {
						$("#caterId").val(req.data.id);
						$("#caterName").val(req.data.name);
						$("#myModal1").modal("show");
					} else {
						bootbox.alert("服务器异常！");
					}
	...
	//保存 餐饮分类
	function saveCater() {
		var caterName = $("#caterName").val();
		if(caterName == null || caterName == ''){
			valTips("caterName","请输入餐饮分类！");
			return;
		}

		$.post("admin/roamMall/cater/save",$("#myform1").serialize(),function(res){
			if('exists'==res.status){
				valTips('caterName', '该餐饮分类已存在！');		
	...
	function deleteCater(id) {
		bootbox.confirm("确定要删除吗?", function(result) {
			if (result) {
				$.ajax({
					url : "admin/roamMall/cater/delete",
					post : "post",
					data : {
						"id" : id
					},
