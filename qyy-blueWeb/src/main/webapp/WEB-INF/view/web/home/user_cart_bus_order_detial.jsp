...
<div class="wrapper inner">
	...
	<div class="content_wrap pb90">
		<div class="content order-manage">
			<div class="breadcrumb">当前位置：<a href="pc/home">首页</a><a href="pc/user/order/depthList">订单管理</a><a href="pc/user/order/rentList">快旅出行</a><a href="pc/user/order/rentList?tabIndex=2">班车</a><span>订单详情</span></div>
			<div class="whitebg pb20">
				...
				<div class="order-info pt20 pb20 clearfix">
					...
					<div class="ml10 fl">
						<p class="f16 dark" style="line-height:113px;">${map.busMap.startStore }-${map.busMap.endStore }班车</p>
					</div>
					<div class="info-right fr rel">
						<i class="iicon arrow abs"></i>
					</div>
				</div>
				<h4 class="orangeh4">班车基本信息</h4>
				<div class="paramitem">
					<p>
						<span class="carparam">
							<i class="i-icon1"></i>品牌：<span>${map.busMap.brand }</span>
						</span>
						<span class="carparam">
							<i class="i-icon5"></i>座位数：<span>${map.busMap.seating }</span>
						</span>
						<span class="carparam">
							<i class="i-icon3"></i>年代：<span>${map.busMap.car_year }</span>
						</span>
					</p>
				</div>
				<h4 class="orangeh4">班车出车信息</h4>
				<div class="paramitem lesspace mt10">
					<span class="carparam">
						<i class="i-icon23"></i>发车时间：<span>${map.busMap.start_time }</span>
					</span>
				</div>
				<div class="paramitem lesspace">
					<span class="carparam">
						<i class="i-icon23"></i>预计到达时间：<span>${map.busMap.to_time }</span>
					</span>						
				</div>
				<div class="paramitem lesspace">
					<span class="carparam">
						<i class="i-icon24"></i>发车地点：<span>${map.busMap.startStore }</span>
					</span>
				</div>
				<div class="paramitem lesspace mb10">
					<span class="carparam">
						<i class="i-icon25"></i>目的地：<span>${map.busMap.endStore }</span>
					</span>
				</div>
				...
				<div class="clearfix pt20 pb20 borderbs1">
					<div class="fr"><span class="grey mr10" id="carPriceDetial"> 139元/人 X 4天=</span><span class="lorange">￥<span class="f24 bold" id="carPrice">556</span></span></div>
...
本服务按天计费，超过4小时以上按1天计费，超时4小时以内不另计费。  <i class="iicon i-arrow"></i></div></span>
				...
				<div class="borderttype2 spanstyle pl10 pt20">
					<p><span class="item">订单编号：<span>${map.busMap.order_code }</span></span></p>
					<p id="payId"><span class="item">支付宝单号：<span>${map.busMap.pay_order_id }</span></span></p>
					<p><span class="item">成交时间：<span id="createTime">${map.busMap.create_time.getTime() }</span></span></p>
					
					<div id="refundDiv">
						<p><span class="item">退款时间：<span id="refundTime">${map.busMap.refund_time.getTime() }</span></span></p>
						<p><span class="item">备注：<span>${map.busMap.refund_reamrk }</span></span></p>
						<p><span class="item">真实姓名：<span>${map.busMap.person_name }</span></span></p>
						<p><span class="item">退款银行名称：<span>${map.busMap.bank_name }</span></span></p>
						<p><span class="item">退款银行卡号：<span>${map.busMap.bank_card }</span></span></p>
					</div>
				</div>
				<div class="pb20 pt20 tr">
					<span class="f16 mr10 grey9">合计</span><span class="lorange f16">￥<span class="f30 bold">${map.busMap.real_price }</span></span>
				</div>
...
$(function(){
	...
	startDate.setTime(parseInt("${map.busMap.start_time.getTime()}"));
	
	//初始化订单状态
	var orderType="${map.busMap.order_state}";
	var orderState="";
	...
	var peopleNumber=parseInt("${map.busMap.goods_count}");
	var realPrice=parseFloat("${map.busMap.real_price}");
	var safePrice=parseFloat("${map.busMap.safe_price}");
...
function doCanleOrder(id,type){
	$.post("pc/user/order/canelOrder",{id:id},function(data){
		if(data.status=="success"){
			alertDialog("取消成功");
			$(".popup-case .ok").one("click",function(){
				alertReload()
...
function doDeleteOrder(id){
	$.post("pc/user/order/deleteOrder",{id:id},function(data){
		if(data.status=="success"){
			alertDialog("删除成功");
			$(".popup-case .ok").one("click",function(){
				window.location.href="pc/user/order/rentList"
			});
		}else if(data.status=="error"){
...
function payOrder(id,order_type,real_price){
	window.location.href="allowAccess/orders/toPay?orderId="+id+"&orderType="+order_type+"&realPrice="+real_price;
}
