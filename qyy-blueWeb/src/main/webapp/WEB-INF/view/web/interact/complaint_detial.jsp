...
<div class="wrapper inner">
	...
	<div class="content_wrap pb90">
		<div class="content">
		    <div class="breadcrumb">当前位置：<a href="">首页</a><a href="pc/interact/home">互动社区</a><a href="pc/complaint/index">吐槽专区 </a><span>吐槽详情</span></div>
			<div class="whitebg">
			    <div class="pd20">
				  <div class="order">
				     <div class="review clearfix">
					    <div class="view-let">
						  <img src="${complaint.head_img }"/>
						...
						<div class="view-rgt">
						  <h3 class="v-title"><span class="grey fr"><i class="iicon i-clock"></i><span id="createTime">2015-6-1    21:30</span></span>${complaint.nickname}</h3>
						  <p>${complaint.content }</p>
						  <c:if test="${complaint.isDelete==0 }">
				...
				<div class="all-comment">
			       <p><span class="i-comment mr10"><i class="icon"></i>解答（${complaint.replyCount}）</span></p>
...
<div class="innerdiv pd20 tc">
	...
	<button class="Btn" name="case-ok" onclick="confirmDelete(${complaint.id})">确认</button>
...
var complaintId=parseInt("${complaint.id}");
var userId=parseInt("complaint.userId");
var ruleDlg ;
var confirmDlg;
var goUrl="";
var tempReportId=0;
...
$(function(){
	...
	$("#createTime").html(getTimeTxt(parseInt("${complaint.create_time.getTime()}"),16));
	
	var iicon='<i class="iicon i-level${complaint.p_level_name }"></i>';
	var badgeNumber=parseInt("${complaint.badgeNumber}");
	$("#rank").empty();
...
function initReply(){
	$.post("<%=basePath %>pc/complaint/replyList",{pageNo:pageNo,complaintId:complaintId},function(data){
		if(data.status=='success'){
			...
			$(".but").click(function(){
				...
				$.post("<%=basePath %>pc/complaint/addReply",data,function(data){
					if(data.status=="success"){
						resetDialog("回复成功");
						ruleDlg.showDlg();
...
function initSecondary(obj){
	...
	$.post("<%=basePath %>pc/complaint/secondaryReply",data,function(data){
		var list=data.list;
...
function praiserReply(obj){
	var replyId=$(obj).attr("_id");
	$.post("<%=basePath %>pc/complaint/praiseReply",{replyId:replyId},function(data){
		if(data.status=="success"){
			resetDialog("点赞成功");
...
function addReply(){
	...
	var toUserId=parseInt("${complaint.userId}");
	var userId=parseInt("${userId}");
	var data={content:content,toUserId:toUserId,toComplaintId:complaintId,userId:userId,rootId:0};
	if(content==""){
		valTips($("#reply"), "请输入内容");
		return;
	}
...
function confirmDelete(id){
	$.post("<%=basePath %>pc/complaint/delete",{complaintId:complaintId,userId:userId},function(data){
		if(data.status=='success'){
			resetDialog("已删除该文章");
