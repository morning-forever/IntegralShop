...
<div class="wrapper inner">
	...
	<div class=" content_wrap pb90">
		<div class="content">
		  <div class="p_result">
		    <img src="static/web/images/l_face.png"/><p>
		 		支付失败<c:if test="${payState eq 'payed' }">,您已经支付过此订单，请不要重复支付。</c:if>   
		   </p>
			<div class="buttn pt20">
				...
				<c:if test="${orderType != 'depth' }">
					<c:if test="${payState eq 'payed' }"><a class="orangebg" href="<%=basePath %>pc/user/order/rentList">返回订单列表</a></c:if>
					<c:if test="${payState != 'payed' }"><a class="orangebg" href="<%=basePath %>pc/user/order/orderDetial/${orderId }">重新支付</a><a href="#">取消支付</a></c:if>
				</c:if>
			</div>
		  </div>
		</div>				
