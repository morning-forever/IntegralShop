...
var remarkContent = "<div class='remarkWarp'>";
remarkContent += "<form><i class='iicon btn-close'></i>";
remarkContent += "<div class='rel'><textarea placeholder='请输入您的意见,200字符以内' style='height:220px;resize: none;' id='message' name='message' notNull='意见不能为空' maxlength='200'></textarea>";
/*remarkContent += "<p class='tips abs'>200字符以内</p>";*/
remarkContent += "</div><div class='info'>";
remarkContent += "<div class='info-item'><label>邮箱</label><input id='email' name='email' notNull='邮箱不能为空'/></div>";
remarkContent += "<div class='info-item'><label>电话</label><input id='phone' name='phone' notNull='电话不能为空'/></div>";
remarkContent += "<div class='info-item'><label>姓名</label><input id='username' name='username' notNull='姓名不能为空' maxlength='20'/></div>";
remarkContent += "<span class='btn' onclick='saveFeedBack()'>确认</span>";
remarkContent += "</div>";
remarkContent += "<form></div>";
$(function() {
	popBoxInit();

	/* 意见反馈弹出框 */
	remarkDlg = new dialogBox({
		contentHtml : remarkContent,
		popupBoxW : 550,
		popupBoxH : 592,
	...
	$(".additional .remark").click(function() {
		$("h3.popup-title,span.btn-span.btn-close").remove();
		console.info($(".remarkWarp form"));
		$(".remarkWarp form")[0].reset();
		remarkDlg.showDlg();
	});
	
	var quickloginHTML = [ '<div class="popup_inner">', '<form action="">', '<div class="form_wrap">',
			'<div class="item"><i class="iicon i-uname"></i><input type="text" class="text" placeholder="请输入您的邮箱／手机号" /></div>',
			'<div class="item" style="margin-bottom:10px;"><i class="iicon i-pwd"></i><input type="text" class="text" placeholder="请输入密码" /></div>',
			'<div class="oitem mb20 pb20 clearfix">', '<a class="fr" href="#">忘记密码？</a>', '<span class="iicon checkbox"></span> 两周内自动登录', '</div>',
			'<button class="btn loginBtn">登录</button>', '</div>', '</form>', '<div class="opart pt10 mb20 tc">',
			'<span class="grey">您还没有黔易游账号？</span><a href="">立刻注册</a>', '</div>', '<div class="tc ologintype">',
			'<a href="#" class="iicon i-weibo"></a>', '<a href="#" class="iicon i-weixin"></a>', '<a href="#" class="iicon i-qq"></a>',
			'</div></div>' ];

	/* 快捷登录弹出框 */
	var quickloginDlg = new dialogBox({
		contentHtml : quickloginHTML.join(""),
		popupBoxW : 490,
		title : "请先登录",
		selfClass : "type3",
		initEvent : function($dom) {
			$dom.find(".checkbox,.checkbox-s,.i-check").click(function() {
...
function saveFeedBack(){
	var flag = true;
	$(".remarkWarp form [notNull]").each(function(){
	...
	if(flag){
		$.post(sys.basePath+'pc/addFeedBack',$(".remarkWarp form").serialize(),function(res){
...
/*
 * 消息提示框 type "info","warning","error" msg 提示消息 使用方法 eg:
 * messageBox("error","谢谢使用！请关闭");
 */
function messageBox(type, msg) {
	...
	if (msg == undefined || msg == '')
		return;
	if ($(".msgbox").length == 0) {
		var box = '<div class="msgbox"><p>' + msg + '</p></div>';
		$("body").append(box);
	} else {
		$(".msgbox p").html(msg);
	}
	var boxH = $(".msgbox").outerHeight(true);
	$(".msgbox").removeClass().addClass("msgbox " + type + "-box");
	$(".msgbox").show().css("top", 0);
...
// 查看商品详情图片
function previewProDetailImg() {
	$('.thumbs img').mouseenter(function() {
		var _idx = $(this).index();
		$('.thumbs img.current').removeClass('current');
		$(this).addClass('current');
		$('.bigimg_wrap img.current').removeClass('current');
		$('.bigimg_wrap img').eq(_idx).addClass('current');
	});
	var imgW = $('.thumbs_list img').eq(0).outerWidth(true);
	$('.thumbs_list').width(imgW * $('.thumbs_list img').length);
	if ($('.thumbs_list img').length > 6) {
		$('.sroll_arrow').show();
	}
}
...
function HorizontalList() {
	var liW = $(".newpro_list li").outerWidth(true);
	var li_len = $(".newpro_list li").length;
	var ulW = liW * li_len;
	var viewW = $(".inner_wrap").width();
	$(".newpro_list").width(liW * li_len);
	$(".newpro_wrap .scroll_left,.newpro_wrap .scroll_right").click(function() {
		if ($(".newpro_list").is(":animated"))
			return;
		var marginL = parseInt($(".newpro_list").css("margin-left"));
		if ($(this).hasClass("scroll_left")) { // 左滑动
			$(".newpro_wrap .scroll_right").removeClass("disabled");
			if (marginL + 186 >= 0) {
				$(".newpro_list").animate({
					"marginLeft" : 0
				}, 500);
			...
			$(".newpro_wrap .scroll_left").removeClass("disabled");
